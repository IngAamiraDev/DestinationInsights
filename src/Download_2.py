...
compare_country_id_2 = ['select_option_1041',
                        'select_option_1032', 'select_option_1033']
...
def load():
    """
    Abre el sitio web de Google Destination Insights y maximiza la ventana.
    """
    start_time = time.strftime("%H:%M:%S")
    start_date = time.strftime("%d/%m/%Y")
    print('Proceso de descarga 2 iniciado: ' + start_date + ' ' + start_time)
...
def page_validation(j):
    """
    Valida los elementos de la página y los gráficos antes de realizar acciones.

    Args:
        j (int): Identificador de la página (1 o 2) para cargas iniciales o posteriores.
    """
    i = 1
    while True:
        try:
            ...
            if page_ready and graphics_ready:
                print("¡La página está lista!")
            break
        except:
            i += 1
            if i <= 8:
                print("¡La carga llevó demasiado tiempo! - Intenta de nuevo")
                driver.refresh()
            else:
                driver.close()
...
def status():
    Files_Download_2.delete_last_file()
    Files_Download_2.download_status()
...
def run():
    ...
    initial_process()
    status()
    download_process_2(x=Files_Download_2.len_country_missing())
    finished()
