...
def extract_cod_country_1(list):
    """
    Extracts the country code from an alternate format of file name.
...
def extract_file(list):
    ...
    Returns:
        str: Extracted base file name without extension.
    """    
    return list[0:-4]
...
def rename_file_download():
    """
    Renames the downloaded files with a specific format.

    The function renames files to '<filename> (1).csv'.
    ...
    for i in range(len(rename_new_file)):
        ...
        source = source_directory + download_file[i]
        destination = source_directory + rename_new_file[i]
...
def len_country_missing():
    """
    Calculates the number of countries missing for download.

    Returns:
        int: Number of countries missing for download.
    """    
    get_files = os.listdir(source_directory)
    flight_csv = [i for i in get_files if i.startswith('FLIGHT', 32, 47)]
    accom_csv = [i for i in get_files if i.startswith('ACCOMMODATION', 32, 54)]
...
def download_status_path():
    ...
    accom_csv = [i for i in get_files_accomm if i.startswith(
        'ACCOMMODATION__30_ (1)', 32, 54)]
    download_flight = [extract_cod_country_1(i) for i in flight_csv]
    download_accomm = [extract_cod_country_1(i) for i in accom_csv]
    download_file = [i for i in download_flight if i in download_accomm]
    download_missing = [
        i for i in primary_country_cod if i not in download_file]
    if (collections.Counter(download_file) == collections.Counter(primary_country_cod)):
